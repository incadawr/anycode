...
describe("SqliteCodexShadowLog", () => {
  it("round-trips a recorded item through the persistence adapter, stripping itemId from the projection shape", async () => {
    ...
    log.record("thread-1", "exec-1", { turnOrdinal: 0, positionInTurn: 0, command: "echo hi", cwd: "/repo", exitCode: 0, outputHead: "hi\n" });
    ...
    expect(items).toEqual([{ turnOrdinal: 0, positionInTurn: 0, command: "echo hi", cwd: "/repo", exitCode: 0, outputHead: "hi\n" }]);
  ...
  it("record() swallows a persistence failure rather than throwing into the live turn", async () => {
    ...
    expect(() => log.record("thread-1", "exec-1", { turnOrdinal: 0, positionInTurn: 0, command: "echo hi" })).not.toThrow();
    await new Promise((resolve) => setTimeout(resolve, 0));
