...
describe("binaryTrustRefusalOf", () => {
  it("BU1 a report carrying trustRefusal returns it verbatim", () => {
    const report = { status: "error", error: "world-writable", trustRefusal: { binaryPath: "/opt/codex", reason: "world-writable" } };
    expect(binaryTrustRefusalOf(report)).toEqual({ binaryPath: "/opt/codex", reason: "world-writable" });
  });
